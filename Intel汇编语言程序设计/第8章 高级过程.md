...
PROC伪指令在默认使用STDCALL调用约定并带一个或多个参数时，假设过程有n个参数，MASM将为其生成如下人口和退出代码：
...
在源码模块中隐藏过程的另外一种方法就是在文件的头部放置伪指令OPTION PROC:PRIVATE,这时默认所有的过程都变成私有的了，可以使PUBLIC 伪指令指定任何要导出的过程：
