...
间接操作数可以是任何用方括号括起来的任意的32位通用寄存器（EAX,EBXECX,EDX,ESI,EDI,EBP和ESP),寄存器里面存放着数据的偏移。下例中ESI中存放着val1的偏移地址。MOV指令使用间接操作数作为源操作数，此时ESI内的偏移地址被用来进行寻址，该地址处的一个字节被送至AL:
