...
堆栈指针寄存器（ESP)指向最后压入（或添加）到堆栈上的数据。为了说明的方便，我们以只有一个值的堆栈为例。下图中，ESP(extended stack pointer)寄存器中包含十六进制数值00001000,这是最后压入的数值00000006的偏移地址。
